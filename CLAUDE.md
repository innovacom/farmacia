...
- WHATSAPP_TEMPLATE_CODIGO (default `autenticacion`), WHATSAPP_TEMPLATE_CODIGO_LANG (default `es`) — plantilla AUTHENTICATION de Meta para el código de acceso sin contraseña a "Mi cuenta" en la tienda web (`tienda.cuenta.service.js`). Reusa las credenciales de WhatsApp Cloud API ya listadas arriba; sin ellas configuradas, `/tienda/cuenta/codigo` responde 503 y el resto de la tienda sigue funcionando igual (checkout de invitado no se ve afectado).
- GOOGLE_MAPS_API_KEY — opcional; API key de Google Maps Geocoding (SOLO Geocoding, no Distance Matrix) usada por `src/config/geo.js` para convertir la dirección de entrega a domicilio (texto libre, capturada por WhatsApp o en el checkout de `/tienda`) en coordenadas, y así validar el radio de cobertura (`configuracion.radio_cobertura_km`, editable en Configuración → admin, default 2 km) contra `sucursales.latitud/longitud`. Sin esta key (o si la geocodificación falla por cualquier motivo), la validación se omite y el pedido sigue su curso normal — nunca bloquea una venta por un problema técnico. Ver `src/services/cobertura.service.js`.
